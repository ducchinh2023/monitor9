...
measurementId: "G-QBCS461Z14"
  ...
  var tempRef = database.ref("/DuLieuBoard/temperature");
  var humidRef = database.ref("/DuLieuBoard/humidity");
  var lightRef = database.ref("/DuLieuBoard/light");
  var soilRef = database.ref("/DuLieuBoard/PH");
...
$(document).ready(function(){
	...
	database.ref().on("value", function(snap){
	 	Led1Status = snap.val().DuLieuGuiXuongBoard.Quat;
		Led2Status = snap.val().DuLieuGuiXuongBoard.Den;
		Led3Status = snap.val().DuLieuGuiXuongBoard.Bom;
		Led4Status = snap.val().DuLieuGuiXuongBoard.Maiche;
	...
	$(".toggle-btn").click(function(){
		var firebaseRef = firebase.database().ref().child("/DuLieuGuiXuongBoard/Quat");
		if(Led1Status == "1"){
			firebaseRef.set(parseInt("0"));
			Led1Status = "0";
	...
	$(".toggle-btn1").click(function(){
		var firebaseRef = firebase.database().ref().child("/DuLieuGuiXuongBoard/Den");
		if(Led2Status == "1"){
			firebaseRef.set(parseInt("0"));
			Led2Status = "0";
	...
	$(".toggle-btn2").click(function(){
		var firebaseRef = firebase.database().ref().child("/DuLieuGuiXuongBoard/Bom");
		if(Led3Status == "1"){
			firebaseRef.set(parseInt("0"));
			Led3Status = "0";
			lightbulbImg3.src = "water.png";
			let msg = new SpeechSynthesisUtterance("Bơm đã tắt");
            window.speechSynthesis.speak(msg);
		} else {
			firebaseRef.set(parseInt("1"));
			Led3Status = "1";
			lightbulbImg3.src = "water.gif";
			let msg = new SpeechSynthesisUtterance("Bơm đã tắt");
            window.speechSynthesis.speak(msg);
		}
	})
	$(".toggle-btn3").click(function(){
		var firebaseRef = firebase.database().ref().child("/DuLieuGuiXuongBoard/Maiche");
		if(Led4Status == "1"){
			firebaseRef.set(parseInt("0"));
			Led4Status = "0";
		} else {
